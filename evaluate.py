...
class Evaluator(object):
    ...
    def __init__(self, config):
        ...
        if config.test.caching_decoder_outputs:
            self.model.build_test_model_with_caching()
            self.beam_search = self.beam_search_with_caching
        else:
    ...
    def beam_search_with_caching(self, X):
        """
        Beam search with batch inputs.
        Args:
            X: A 2-d array with size [n, src_length], source sentence indices.
        ...
        preds = np.ones([batch_size * beam_size, 1], dtype=INT_TYPE) * 2  # [[<S>, <S>, ..., <S>]], shape: [batch_size * beam_size, 1]
        scores = np.array(([0.0] + [-inf] * (beam_size - 1)) * batch_size)  # [0, -inf, -inf ,..., 0, -inf, -inf, ...], shape: [batch_size * beam_size]
        cache = np.zeros([batch_size * beam_size, 0, self.config.num_blocks, self.config.hidden_units])
        for i in xrange(min(config.test.max_target_length, X.shape[1] + 50)):
            # Whether sequences finished.
            bias = np.equal(preds[:, -1], 3)   # </S>?
            # If all sequences finished, break the loop.
            ...
            # Expand the nodes.
            last_k_preds, last_k_scores, cache = \
                self.sess.run([self.model.k_preds, self.model.k_scores, self.model.decoder_cache],
                              feed_dict={self.model.encoder_output: encoder_output,
                                         self.model.decoder_cache_pl: cache,
                                         self.model.decoder_input: preds})  # presd & scores: [batch_size * beam_size, beam_size]
            ...
            last_k_preds = last_k_preds.flatten()[k_indices]
            preds = preds[k_indices // beam_size]
            cache = cache[k_indices // beam_size]
            preds = np.concatenate((preds, last_k_preds[:, None]), axis=1)  # [batch_size * beam_size, i]
